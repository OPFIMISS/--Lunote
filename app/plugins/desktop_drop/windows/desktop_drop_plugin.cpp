...
namespace {
    ...
    HRESULT DesktopDropTarget::Drop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
        ...
        if (list.empty()) {
            ...
            if (pDataObj->QueryGetData(&text_format) == S_OK &&
                pDataObj->GetData(&text_format, &text_medium) == S_OK) {
                auto text_ptr = static_cast<const wchar_t *>(GlobalLock(text_medium.hGlobal));
                if (text_ptr != nullptr) {
                    const std::string text = ws2s(std::wstring(text_ptr));
                    GlobalUnlock(text_medium.hGlobal);
                    ReleaseStgMedium(&text_medium);
                    channel_->InvokeMethod(
                            "performOperationText",
